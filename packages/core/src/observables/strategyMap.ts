...
/** @internal */
export const strategyMap$ = new BehaviorSubject<Map<string, IStrategy>>(new Map([]));
export const strategyMapø = strategyMap$.pipe<Map<string, IStrategy>>(share());
...
account$.pipe(withLatestFrom(strategyMap$)).subscribe(([account]) => {
...
const _updateStrategy = async (
  ...
  if (account) {
    ...
    const accountStrategyPercent = mulDecimal(divDecimal(accountBalance, strategyTotalSupply || '0'), '100');
    accountData = {
      ...strategy,
      accountBalance,
