...
const borrow = (amount, collateralAmount, vault, getValuesFromNetwork = true // Get market values by network call or offline calc (default: NETWORK)
    ...
    const vaultMap = observables_1.vaultMap$.value;
    const selected = observables_1.selected$.value;
    ...
    const getValidatedVault = (v) => {
        if (v) {
            ...
            (0, messages_1.sendMsg)({ message: 'Vault ID provided, but not recognised.', type: messages_1.MessageType.WARNING, origin: 'Borrow()' });
            throw new Error('Vault ID provided, but was not recognised.');
            // return undefined //TODO: should pass this instead of throwing error? 
        }
        (0, messages_1.sendMsg)({ message: 'No Vault ID provided. Creating a new Vault...', type: messages_1.MessageType.INFO, origin: 'Borrow()' });
        return undefined;
    };
    ...
    const _expectedFyToken = getValuesFromNetwork
        ...
        : (0, ui_math_1.buyBase)(series.baseReserves, series.fyTokenReserves, _amount, series.getTimeTillMaturity(), series.ts, series.g2, series.decimals);
    ...
    const calls = [
        ...
        /* If convex-type collateral, add vault using convex ladle module */
        {
            ...
            ignoreIf: vaultId !== utils_1.BLANK_VAULT || !isConvexCollateral,
        },
        {
            operation: types_1.LadleActions.Fn.SERVE,
            args: [vaultId, serveToAddress(), _collAmount, _amount, _expectedFyTokenWithSlippage],
            ignoreIf: false,
        },
        ...removeEthCallData,
    ];
